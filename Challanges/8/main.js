...
function getButton(content, classname) {
...
function init() {
  ...
  for (let i = 0; i < numberOfCalls; i++) {
    axios.get("https://randomuser.me/api/").then((response) => {
      ...
      const respdataJob = gender[Math.floor(Math.random()*gender.length)];
      ...
      const editButton = getButton(icons.edit, ['edit-button']);

      const saveButton = getButton(icons.save, ['Save-button', 'disable'] );
